...
public class CharacterTokenizerImpl {
	   ...
	   */
	  public CharacterTokenizerImpl(java.io.Reader in) {
	    this.zzReader = in;
	  }
	    ...
	    while (true) {
	      ...
	      zzForAction: {
	        while (true) {
	          ...
	          if (zzNext == -1) break zzForAction;//如果是特殊字符，则跳出zzForAction循环，不记录此字符
	          zzState = zzNext;

	          zzAttributes = zzAttrL[zzState];
	          zzAction = zzState;
	          zzMarkedPosL = zzCurrentPosL;
	          break zzForAction;//如果匹配到非英文数字字符，则跳出循环，但是会记录此字符
	        }
	      }
